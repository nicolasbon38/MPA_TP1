...
```{R}
#On définit une fonction qui renvoie le nombre de 1 dans y1...yc-1, et dans yc...yn et les thetas dans un échantillon y coupé en c:
...
frequences_sans_coupure <- function(y){
  ...
  t2 <- compteur / (length(y) + 1)
  return(c(0, 0, t2, compteur))
}
...
n <- length(y)
c <- floor(runif(1, 1, n + 1)) #c au au hasard


for (iteration_algo in seq(1:10)){
  #on identifie theta1 et theta2 :
  frequence = frequences(y, c)
  ...
  for(i in seq(from=2,to=n)){
    pi = probas[i - 1] * t1^y[i - 1] * ((1 - t1)^(1 - y[i-1]) / (t2 ^ y[i - 1])) * (1 - t2)^(1 - y[i - 1])*p1
    probas = c(probas, pi)
