...
On reprend donc le même procédé qu'a la question deux, en choisissant le $\theta$ le plus probable qui correspond ici à la moyenne ie environ 11.
...
```
Il est difficle de comparé les valeurs calculées aux valeurs de départ car il y a peu de valeurs de départs (à cause de la capacité de calculs limitées)
...
Gibbs.a.K.classes <- function(K){
  ...
  matrice.probas_z = matrix(nrow=length(y), ncol=K)
  ...
  lines(x_graph, y_graph)
  print(matrice.probas_z)
  barplot(matrice.probas_z, beside=FALSE, horiz=FALSE, col=c("blue", "green", "red"), xlim=c(0, 210))
}
...
La simulation à 3 classes paraît être la plus adaptée.